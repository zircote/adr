...
2. **Filter and rank**: narrow by `status`, `category`, `tags`, `technologies`,
   ...
   When reporting results, **briefly state why each excluded record was dropped**
   (e.g. "excluded: category ui, not database" or "excluded: status proposed,
   not accepted") — not just which records matched.
